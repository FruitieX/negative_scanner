        ...
        if !pause && !MANUAL_MODE {
            match state.current {
                    ...
                    match last_cwm {
                        Some(last_cwm) => {
                            ...
                            if let Some(last_seen_x) = last_seen_x {
                                if last_seen_x >= stop_pos && last_cwm.end_x < stop_pos {
                                    scanner.stop(true);
                                    state.set(ScannerState::AligningFrame {
                        ...
                        None => {
                            ...
                            println!("Unable to find frame gap, taking photo");
                            scanner.stop(false);
                            state.set(ScannerState::TakingPhoto {
                                init_timestamp: SystemTime::now(),
                            })
                ...
                ScannerState::SkipToNextFrame { init_timestamp } => {
                    ...
                    if init_timestamp.elapsed().unwrap().as_secs_f64() < 3.0 {
                        continue;
                    }
